...
## NOTE: in order to make tis work you need to copy the file "run_run_quantisnp.sh" from here ($WKDIR/01_initial_call/run_QuantiSNP/) to the QuantiSNP directory ($QUANTISNP/quantisnp/linux64/run_run_quantisnp.sh)
...
### NEED TO CHANGE "opt$input" to "opt$data" & "opt$output" to "opt$result"
...
if (!file.exists(file.path(path_quantisnp, "quantisnp/linux64/run_run_quantisnp.sh"))){
	stop("Place the additional script in the correct location ($QUANTISNP/quantisnp/linux64/run_run_quantisnp.sh)")
	}
...
## in ornder to pass the arguments from qsub to quantisnp it is necessary to first use global variables with the "-v" flag and then use an intermediate script (run_run_quantisnp.sh) to reconvert the variables in a valid command for quantisnp
## note that the flags that do not require any input (e.g. --doXcorrect) where moved from "cmd" (here) to run_run_quantisnp.sh
